...
public class TestR4_Simulation {
	...
	public void testSimpleElements(){
		...
		assertEquals("source",obs.typeOf("Src"));
	...
	public void testSimpleElementsClosedTap(){
		...
		assertEquals("Wrong input flow of 'Tap'", flow, inTap, 0.01);
		assertEquals("Wrong output flow of 'Tap'", 0, outTap, 0.01);
		assertEquals("Wrong input flow of 'Sink'", 0, inSink, 0.01);
	}
	...
	private static class StoreObserver implements SimulationObserver {
		...
		String typeOf(String name) {
			if(!events.containsKey(name.toLowerCase())) return null;
			return events.get(name.toLowerCase()).type;
		}

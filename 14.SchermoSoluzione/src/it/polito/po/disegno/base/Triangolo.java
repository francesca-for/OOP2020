...
public class Triangolo implements Figura {
	...
	public void disegna(Schermo s) {
		...
		for (int i = 0; i < h; i++) {
			...
			if(i==h-1)  // controllo aggiuntivo per essere sicura di stampare il vertice 
				lastJ=1;
			...
			lastJ-=((float)l-1)/((float)h-1);  // per vedere di quanto diminuire ogni riga per stampare la diagonale 
		}
